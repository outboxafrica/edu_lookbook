...
module.exports = {
  createUserProfile: async (req, res) => {
    ...
    try {
      ...
      if (user) {
        ...
        res
          .status(200)
          .json({ Message: "Successfully Added A Profile", Profile: saved });
      }
    } catch (err) {
      //throw an error
      res.status(403).json({
        Message: "Something Went Wrong >> Unable to create Profile",
        Error: err,
      });
    }
  },
  ...
  updateUserProfile: async (req, res) => {
    ...
    try {
      ...
      if (user) {
        res.status(403).json({
          Message: "Username provided Already Taken >> Enter Unique Username",
        });
      } else {
        ...
        res
          .status(200)
          .json({ Message: `${profile.username}'s Profile has been Updated` });
      }
    } catch (err) {
      //Throw an error Message
      res.status(404).json({
        Message: `Profile of given ID >> ${req.params.profileId} << was not Found`,
        Error: err,
      });
    }
  },
  deleteUserProfile: async (req, res) => {
    try {
      ...
      res.status(200).json({
        message: `${deleteUser.username}'s Profile has been deleted successfully`,
      });
    } catch (error) {
